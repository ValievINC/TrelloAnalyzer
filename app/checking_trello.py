...
def get_results(source: str):
    csv_input = pd.read_csv(source, encoding='utf-8')
    csv_input['Result'] = '0%'

    for i in range(len(csv_input['Link'])):
...
def create_new_csv(source_name: str, result_name: str):
    get_results(source_name).to_csv(result_name, index=False)


def create_new_excel_file(source_name: str, result_name: str):
    get_results(source_name).to_excel(result_name, index=False)
